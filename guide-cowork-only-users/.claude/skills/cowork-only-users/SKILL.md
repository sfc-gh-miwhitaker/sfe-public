...
description: "Admin runbook for provisioning Snowflake CoWork-only users. Covers CORTEX_AGENT_USER role, ALLOWED_INTERFACES, CoWork object setup, single and bulk user provisioning, SCIM attribute, private connectivity, and IdP redirect. Use when working on this guide or helping an admin provision CoWork users."
---
...
- Private connectivity URLs must be regionless (`si-<org-acct>.privatelink.snowflakecomputing.com`) — region-specific URLs fail.
- SCIM `allowedInterfaces` attribute is an alternative to `ALTER USER` for IdP-managed provisioning.
- The `SNOWFLAKE_INTELLIGENCE.AGENTS` schema for managing agent visibility is deprecated — use the CoWork object instead.
