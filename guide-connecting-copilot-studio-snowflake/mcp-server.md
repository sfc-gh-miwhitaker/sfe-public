...
> **Model selection:** Never hardcode a model name. Run `SHOW CORTEX BASE MODELS` and filter on `lifecycle_status = 'GA'` with no `legacy_date` set. Models deprecate monthly — the safe path is to omit the `models:` block entirely and let Snowflake auto-select, then pin a specific GA model only if your testing shows a quality improvement.
...
CREATE OR REPLACE AGENT <DATABASE>.<SCHEMA>.COPILOT_AGENT
  ...
  instructions:
    response: "You are a data analytics assistant. Answer questions about business data using the available tools. Be concise and accurate."
    orchestration: "For structured data questions use the analyst tool. For document or policy questions use the search tool."
  ...
  tools:
    ...
    - tool_spec:
        type: "data_to_chart"
        name: "data_to_chart"
        description: "Generates visualizations from data returned by the analyst tool"
  ...
  tool_resources:
    analyst:
      ...
      execution_environment:
        ...
        query_timeout: 60
    search:
      name: "<DATABASE>.<SCHEMA>.MY_SEARCH_SERVICE"
      max_results: "5"
  $$;
```
...
| Agent errors after model was working | Model entered LEGACY/EOL state | Run `SHOW CORTEX BASE MODELS` and pick a GA model; omit the `models:` block to auto-select |
