/*
  guide-shopify-multistore-snowflake — sql/openflow/01_core_snowflake.sql
  ...
  PREREQUISITE
    sql/shared/01_store_registry.sql has been run.
...
-- Read access to the shared registry, which drives the landing schemas and
-- the generated analytics layer.
GRANT USAGE  ON DATABASE SHOPIFY_CONTROL          TO ROLE OPENFLOW_ADMIN;
GRANT USAGE  ON SCHEMA   SHOPIFY_CONTROL.META     TO ROLE OPENFLOW_ADMIN;
GRANT SELECT ON ALL TABLES    IN SCHEMA SHOPIFY_CONTROL.META TO ROLE OPENFLOW_ADMIN;
GRANT SELECT ON ALL VIEWS     IN SCHEMA SHOPIFY_CONTROL.META TO ROLE OPENFLOW_ADMIN;
GRANT CREATE VIEW             ON SCHEMA SHOPIFY_CONTROL.META TO ROLE OPENFLOW_ADMIN;
GRANT UPDATE ON TABLE SHOPIFY_CONTROL.META.STORE_REGISTRY    TO ROLE OPENFLOW_ADMIN;
