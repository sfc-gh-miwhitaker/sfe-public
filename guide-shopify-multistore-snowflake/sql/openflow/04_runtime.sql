/*
  guide-shopify-multistore-snowflake — sql/openflow/04_runtime.sql
  ...
  SIZING (facts, then judgment)
    ...
    and a daily bulk-plus-incremental Shopify flow is much lighter than a
    continuous CDC stream, so the CDC number is a floor, not a ceiling.
    ...
    event table before adding more; prefer a second runtime for blast radius
    over scaling one runtime to its ceiling.
