/*
  guide-shopify-multistore-snowflake — sql/native/03_pull_procedure.sql
  Pair-programmed by SE Community + Cortex Code
  Expires: 2026-12-21

  PURPOSE
    The deterministic data path: token, bulk operation, poll, download, stage,
    COPY, log. No LLM is involved at run time.

  SHOPIFY API VERSION
    Pinned explicitly to 2026-01 (see API_VERSION below). Do not float this.
    What 2026-01 changed, and why each matters here:

      * Five concurrent bulk QUERY operations per app per shop (and five bulk
        mutations). Earlier versions allowed one of each type. The limit is per
        APP, so another vendor's integration on the same store does not consume
        your slots.
      * Poll with bulkOperation(id:), not currentBulkOperation. With five
        possible in flight, "current" is ambiguous and cannot identify yours.
      * Object grouping is OFF by default for bulkOperationRunQuery. Never
        assume a child record follows its parent in the JSONL. Every child here
        is joined to its parent by __parentId, which is order-independent.
      * File uploads up to 100 MB.

    2026-01 is supported until at least 2027-01-01. Roll the version through
    one-store qualification before the fleet (coco/MAINTENANCE_PLAYBOOK.md).

  BULK QUERY LIMITS
    At most 5 connections and 2 levels of nesting per bulk query. The order
    query below uses 2 connections (lineItems, and the fulfillments array).

  GENERATED SECTIONS
    STORE_BINDINGS and the SECRETS clause are generated by
    tools/generate_store_bindings.py. Regenerate them when stores change; the
    aliases are fixed at procedure creation time. Secret values stay inside
    Snowflake and are readable only by this handler.
...
SECRETS = (
  -- GENERATED by tools/generate_store_bindings.py
...
# Pinned explicitly. 2026-01 is the floor: it is the version that allows five
# concurrent bulk query operations per app per shop and requires polling by
# operation ID. See the file header before changing this.
API_VERSION = "2026-01"
POLL_SECONDS = 30
MAX_POLLS = 240
...
# Poll by ID. currentBulkOperation cannot identify one of five concurrent
# operations and must not be used on 2026-01 or later.
STATUS_QUERY = """
...
def pull_store(session: Session, store_key: str, object_name: str, since_ts):
    ...
    if binding is None:
        raise ValueError(
            f"CONFIG: no generated binding for {store_key}. "
            "Fix: add the store to config/stores.json and regenerate bindings."
        )
    ...
    try:
        ...
        with requests.Session() as http:
            ...
            if result_url:
                # Result URLs expire after seven days; download immediately.
        ...
        if result_stream is not None:
            # SQL PUT is unsupported in stored procedures; put_stream is the
            # only supported way to write these bytes to a stage.
...
-- All-store loop. Failures are isolated per store and object: one store does
-- not prevent the rest from running, but the procedure still raises so the task
-- is marked failed.
...
BEGIN
  STORES := (SELECT STORE_KEY FROM SHOPIFY_CONTROL.META.STORE_REGISTRY
...
-- Qualification: extract one day for one store with the task still suspended
-- and the store still inactive, then record evidence per gate. This is why the
-- native path can prove a store before promoting it; the Openflow path cannot
...
BEGIN
  ...
  FROM SHOPIFY_CONTROL.META.STORE_REGISTRY WHERE STORE_KEY = :STORE_KEY;
  ...
  COMPARISON AS (
    ...
    FROM ACTUAL A
    JOIN SHOPIFY_CONTROL.META.RECONCILIATION_BASELINE B
