...
RETURNS NUMBER
COMMENT = 'DEMO: Metric value validity percent | Author: SE Community | Expires: 2026-11-06'
AS
$$
  SELECT ROUND(
...
RETURNS NUMBER
COMMENT = 'DEMO: Session duration validity percent | Author: SE Community | Expires: 2026-11-06'
AS
$$
  SELECT ROUND(
...
CREATE OR REPLACE VIEW SNOWFLAKE_EXAMPLE.DATA_QUALITY.V_ATHLETE_PERFORMANCE
  COMMENT = 'DEMO: Cleaned athlete performance view (valid metric_value only) | Author: SE Community | Expires: 2026-11-06'
AS
SELECT
  athlete_id,
  ngb_code,
...
CREATE OR REPLACE VIEW SNOWFLAKE_EXAMPLE.DATA_QUALITY.V_FAN_ENGAGEMENT
  COMMENT = 'DEMO: Cleaned fan engagement view (valid session_duration only) | Author: SE Community | Expires: 2026-11-06'
AS
SELECT
  engagement_id,
  fan_id,
...
CREATE OR REPLACE VIEW SNOWFLAKE_EXAMPLE.DATA_QUALITY.V_DATA_QUALITY_METRICS
  COMMENT = 'DEMO: Data quality metrics view | Author: SE Community | Expires: 2026-11-06'
AS
SELECT
  metric_date,
  table_name,
...
CREATE OR REPLACE VIEW SNOWFLAKE_EXAMPLE.DATA_QUALITY.V_QUALITY_SCORE_TREND
  COMMENT = 'DEMO: Quality score trend view | Author: SE Community | Expires: 2026-11-06'
AS
SELECT
  metric_date,
  table_name,
