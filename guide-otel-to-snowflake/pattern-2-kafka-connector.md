...
- Do not copy v3 configuration from older blog posts. The property names differ and you will be
  migrating within the window.
- The deprecation timeline is *planned*, not announced. Verify current status before you cite
  dates to anyone. See
...
- **`StringConverter`, not `JsonConverter`.** The Collector already produced OTLP JSON. A
