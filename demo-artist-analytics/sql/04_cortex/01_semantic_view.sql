...
CREATE OR REPLACE SEMANTIC VIEW SNOWFLAKE_EXAMPLE.SEMANTIC_MODELS.SV_ARTIST_ANALYTICS
  ...
  FACTS (
    -- Streams
    PRIVATE streams.streams AS raw_streams
      COMMENT = 'Raw daily stream count per platform',
    PRIVATE streams.saves AS raw_saves
      COMMENT = 'Raw daily save count per platform',
    PRIVATE streams.listeners AS raw_listeners
      COMMENT = 'Raw daily listener count per platform',

    -- Social
    PRIVATE social.impressions AS raw_impressions
      COMMENT = 'Raw daily impressions per social platform',
    PRIVATE social.engagements AS raw_engagements
      COMMENT = 'Raw daily engagements per social platform',
    PRIVATE social.shares AS raw_shares
      COMMENT = 'Raw daily shares per social platform',

    -- Income
    income.stream_royalties AS stream_royalties
      COMMENT = 'Daily streaming royalty income in USD — computed from actual stream counts × platform royalty rates',
    income.merch_estimate AS merch_estimate
      COMMENT = 'Estimated daily merchandise revenue — spikes near show dates',
    income.sync_licensing AS sync_licensing
      COMMENT = 'Sync licensing payments — music placed in TV, ads, or film. Occasional lump sums.',
    income.total_income AS total_income
      COMMENT = 'Total daily income from all sources',

    -- Shows
    shows.ticket_capacity AS ticket_capacity
      COMMENT = 'Venue capacity in tickets',
    shows.ticket_price AS ticket_price
      COMMENT = 'Ticket face value in USD',
    shows.days_until_show AS days_until_show
      COMMENT = 'Days remaining until the show date',
    shows.baseline_daily_engagements AS baseline_daily_engagements
      COMMENT = 'Average daily social engagements in the 30-day baseline window (45-15 days before show)',
    shows.pre_show_daily_engagements AS pre_show_daily_engagements
      COMMENT = 'Average daily social engagements in the 14-day pre-show window',
    shows.momentum_score AS momentum_score
      COMMENT = 'Momentum score: 100 = flat vs baseline; >100 = building; <100 = cooling. NULL if pre-show window has not started yet.'
  )
  ...
  DIMENSIONS (
    ...
    streams.platform_name AS streaming_platform
    ...
    streams.platform_type AS platform_type
      WITH SYNONYMS = ('platform model', 'subscription type')
      COMMENT = 'Business model of the platform'
      SAMPLE_VALUES ('Subscription', 'Ad-Supported')
      IS_ENUM,

    -- Social
    social.metric_date AS social_date
  ...
  AI_VERIFIED_QUERIES (
    q_total_streams_last_month AS (
      ...
      SQL 'SELECT
           ...
           FROM SNOWFLAKE_EXAMPLE.ARTIST_ANALYTICS.FACT_DAILY_STREAMS
           WHERE stream_date >= DATE_TRUNC(''month'', DATEADD(''month'', -1, CURRENT_DATE()))
             AND stream_date <  DATE_TRUNC(''month'', CURRENT_DATE())'
    ),
    ...
    q_streams_by_platform AS (
      ...
      SQL 'SELECT
           ...
           FROM SNOWFLAKE_EXAMPLE.ARTIST_ANALYTICS.FACT_DAILY_STREAMS fds
           JOIN SNOWFLAKE_EXAMPLE.ARTIST_ANALYTICS.DIM_PLATFORM dp
               ON dp.platform_id = fds.platform_id
           WHERE fds.stream_date >= DATEADD(''day'', -30, CURRENT_DATE())
    ...
    q_total_income_this_month AS (
      ...
      VERIFIED_BY '(STEWARD = se_community)'
      SQL 'SELECT
               ROUND(SUM(stream_royalties), 2) AS streaming_royalties,
               ROUND(SUM(merch_estimate), 2)   AS merch_income,
               ROUND(SUM(sync_licensing), 2)   AS sync_income,
               ROUND(SUM(total_income), 2)     AS total_income
           FROM SNOWFLAKE_EXAMPLE.ARTIST_ANALYTICS.FACT_INCOME
    ...
    q_social_engagement_by_platform AS (
      ...
      SQL 'SELECT
           ...
           FROM SNOWFLAKE_EXAMPLE.ARTIST_ANALYTICS.FACT_SOCIAL_METRICS fsm
           JOIN SNOWFLAKE_EXAMPLE.ARTIST_ANALYTICS.DIM_SOCIAL_PLATFORM dsp
               ON dsp.social_platform_id = fsm.social_platform_id
           WHERE fsm.metric_date >= DATEADD(''day'', -30, CURRENT_DATE())
    ...
    q_stream_growth_trend AS (
      ...
      VERIFIED_BY '(STEWARD = se_community)'
      SQL 'SELECT
               DATE_TRUNC(''week'', stream_date) AS week_start,
               SUM(streams)                      AS total_streams,
               SUM(listeners)                    AS total_listeners
           FROM SNOWFLAKE_EXAMPLE.ARTIST_ANALYTICS.FACT_DAILY_STREAMS
           WHERE stream_date >= DATEADD(''day'', -60, CURRENT_DATE())
           GROUP BY DATE_TRUNC(''week'', stream_date)
           ORDER BY week_start'
    )
  );
