...
|---|---|---|
| [guide-ad-platform-integrations](guide-ad-platform-integrations/) | Two commonly conflated ad-platform integrations. Google Ads Data Manager with Snowflake as a native first-party source: PAT authentication (not password, not key-pair), no ROLE field so `ROLE_RESTRICTION` is mandatory, the dynamic-Google-Cloud-IP vs network-policy conflict resolved via `TYPE = SERVICE_AGENT`, Google-side hex SHA-256 hashing, quoted Customer Match header aliases, confidential matching on by default, and the 100-record / 540-day / 100M-row limits. Meta's Ads MCP Server (open beta) registered via `EXTERNAL MCP SERVER` with `OAUTH_DYNAMIC_CLIENT`, plus the Cortex Code `mcp.json` path and read-only permission guardrails. Names the Snowflake→Meta activation gap and flags two contradictions inside Google's own docs | Google Ads Data Manager, Customer Match, offline conversions, PAT, secure view, MCP connectors, EXTERNAL MCP SERVER, Cortex Agents, Cortex Code, confidential matching, Openflow |
