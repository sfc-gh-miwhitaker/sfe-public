...
|---|---|---|
| [guide-ad-platform-integrations](guide-ad-platform-integrations/) | Moving advertising data between Snowflake and Google/Meta, split by direction. Outbound: Google Ads Data Manager with Snowflake as a native first-party source — PAT authentication (not password, not key-pair), no ROLE field so `ROLE_RESTRICTION` is mandatory, the dynamic-Google-Cloud-IP vs network-policy conflict resolved via `TYPE = SERVICE_AGENT`, Google-side hex SHA-256 hashing, quoted Customer Match header aliases, and the 100-record / 540-day / 100M-row limits. Inbound: Openflow connectors for Meta Ads and Google Ads (Preview; platform GA) — the three account privileges, egress network rules for `graph.facebook.com` and `googleads.googleapis.com`, execute-as roles, and documented ingestion limits. Part 1 and the Part 2 prerequisites executed live and torn down | Google Ads Data Manager, Customer Match, offline conversions, PAT, secure view, Openflow, Meta Ads connector, Google Ads connector, network rules, external access integration |
