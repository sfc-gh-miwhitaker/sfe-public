...
Trigger phrases: "review this SQL", "check standards", "validate naming", "pre-commit check"

## Workflow

1. **Check SQL quality** — scan for SELECT *, non-sargable WHERE predicates, window functions without QUALIFY, mismatched join types, and missing STATEMENT_TIMEOUT. See `references/standards.md` for the full rules.
