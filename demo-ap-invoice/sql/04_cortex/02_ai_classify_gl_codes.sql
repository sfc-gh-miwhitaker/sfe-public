...
RETURNS VARIANT
LANGUAGE SQL
COMMENT = 'DEMO: Classifies a line item description into GL codes using AI_CLASSIFY (Expires: 2026-08-12)'
AS
$$
DECLARE
    item_desc VARCHAR;
...
BEGIN
    ...
    SELECT ARRAY_AGG(
        OBJECT_CONSTRUCT(
            'label', GL_CODE,
            'description', GL_DESCRIPTION || ' — ' || CATEGORY || ' expenses'
        )
    ) INTO gl_categories
    FROM GL_CODES;
    ...
    SELECT AI_CLASSIFY(
        ...
        :gl_categories,
        {
            'task_description': 'Classify this hospitality invoice line item into the most appropriate GL account code. Consider the nature of the expense — food, linens, IT, maintenance, entertainment, or professional services.'
        }
    ) INTO classification;
...
RETURNS VARCHAR
LANGUAGE SQL
COMMENT = 'DEMO: Batch GL classification for all unclassified line items (Expires: 2026-08-12)'
AS
$$
DECLARE
