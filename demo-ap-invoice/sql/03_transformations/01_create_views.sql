...
CREATE OR REPLACE VIEW PROCESSED_INVOICES
  COMMENT = 'DEMO: Joined invoice data for analytics and export (Expires: 2026-08-12)'
AS
SELECT
    h.INVOICE_ID,
    h.SOURCE_FILE,
...
CREATE OR REPLACE VIEW V_REVIEW_QUEUE
  COMMENT = 'DEMO: Pending review items with invoice context (Expires: 2026-08-12)'
AS
SELECT
    rq.QUEUE_ID,
    rq.INVOICE_ID,
...
CREATE OR REPLACE VIEW V_LINE_ITEMS_ENRICHED
  COMMENT = 'DEMO: Line items with GL code descriptions (Expires: 2026-08-12)'
AS
SELECT
    li.LINE_ID,
    li.INVOICE_ID,
...
CREATE OR REPLACE VIEW V_PIPELINE_METRICS
  COMMENT = 'DEMO: Aggregated pipeline KPIs (Expires: 2026-08-12)'
AS
SELECT
...
CREATE OR REPLACE VIEW V_SPEND_ANALYSIS
  COMMENT = 'DEMO: Spend breakdown by property and vendor (Expires: 2026-08-12)'
AS
SELECT
    h.PROPERTY,
