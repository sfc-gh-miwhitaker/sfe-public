...
CREATE STREAM IF NOT EXISTS INVOICE_HEADER_STREAM
    ...
    APPEND_ONLY = TRUE
    COMMENT = 'DEMO: Captures new invoice extractions for validation pipeline (Expires: 2026-08-12)';
...
RETURNS VARCHAR
LANGUAGE SQL
COMMENT = 'DEMO: Validates new extractions and routes low-scoring invoices to review (Expires: 2026-08-12)'
AS
$$
DECLARE
...
CREATE TASK IF NOT EXISTS VALIDATE_INVOICES_TASK
    ...
    SCHEDULE = '5 MINUTE'
    COMMENT = 'DEMO: Auto-validates new extractions using stream trigger (Expires: 2026-08-12)'
...
RETURNS VARIANT
LANGUAGE SQL
COMMENT = 'DEMO: End-to-end invoice processing - extract, match, classify, score (Expires: 2026-08-12)'
AS
$$
DECLARE
    extraction VARIANT;
...
BEGIN
    -- Step 1: AI_EXTRACT structured fields from PDF (with per-field confidence scores)
    SELECT AI_EXTRACT(
    ...
    -- Step 3: Compute validation score using AI_EXTRACT per-field confidence
    score := (
        COALESCE(extraction:scoring:scores:vendor_name:score::NUMBER(5,2), 0) * 0.20 +
        COALESCE(extraction:scoring:scores:invoice_number:score::NUMBER(5,2), 0) * 0.15 +
        COALESCE(extraction:scoring:scores:invoice_date:score::NUMBER(5,2), 0) * 0.15 +
        COALESCE(extraction:scoring:scores:total_amount:score::NUMBER(5,2), 0) * 0.20 +
        COALESCE(extraction:scoring:scores:po_reference:score::NUMBER(5,2), 0) * 0.10
    );
