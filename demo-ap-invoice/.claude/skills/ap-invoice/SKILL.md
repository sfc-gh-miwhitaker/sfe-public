...
AI_EXTRACT returns per-field confidence scores (`scores => TRUE`, GA Mar 2026). Validation score is a weighted sum of these extraction confidences plus a vendor master fuzzy matching bonus (JAROWINKLER_SIMILARITY).
...
- All objects have `COMMENT = 'DEMO: ... (Expires: 2026-08-12)'`

## Gotchas

- **AI_EXTRACT scores are used for validation.** The `scores => TRUE` parameter (GA Mar 2026) returns per-field confidence. The validation score is a weighted sum of these plus a vendor-match bonus.
