...
-- Grant access so any role can use the agent and its tools
GRANT USAGE ON SCHEMA SNOWFLAKE_EXAMPLE.FINANCIAL_AGENTS TO ROLE PUBLIC;
GRANT USAGE ON SCHEMA SNOWFLAKE_EXAMPLE.SEMANTIC_MODELS TO ROLE PUBLIC;
GRANT SELECT ON ALL TABLES IN SCHEMA SNOWFLAKE_EXAMPLE.FINANCIAL_AGENTS TO ROLE PUBLIC;
GRANT USAGE ON AGENT SNOWFLAKE_EXAMPLE.FINANCIAL_AGENTS.PORTFOLIO_RISK_AGENT TO ROLE PUBLIC;
GRANT USAGE ON CORTEX SEARCH SERVICE SNOWFLAKE_EXAMPLE.FINANCIAL_AGENTS.FACILITY_DOCUMENT_SEARCH TO ROLE PUBLIC;

-- Register agent with Snowflake Intelligence (requires ACCOUNTADMIN for CREATE)
USE ROLE ACCOUNTADMIN;
CREATE SNOWFLAKE INTELLIGENCE IF NOT EXISTS SNOWFLAKE_INTELLIGENCE_OBJECT_DEFAULT;
GRANT USAGE ON SNOWFLAKE INTELLIGENCE SNOWFLAKE_INTELLIGENCE_OBJECT_DEFAULT TO ROLE PUBLIC;
ALTER SNOWFLAKE INTELLIGENCE SNOWFLAKE_INTELLIGENCE_OBJECT_DEFAULT
  ADD AGENT SNOWFLAKE_EXAMPLE.FINANCIAL_AGENTS.PORTFOLIO_RISK_AGENT;
USE ROLE SYSADMIN;
