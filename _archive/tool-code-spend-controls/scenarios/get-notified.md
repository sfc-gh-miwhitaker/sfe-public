...
## Option E: Per-user daily limit approach alerts

Snowflake's per-user daily credit limits (`CORTEX_CODE_CLI_DAILY_EST_CREDIT_LIMIT_PER_USER` / `CORTEX_CODE_SNOWSIGHT_DAILY_EST_CREDIT_LIMIT_PER_USER`) block users when they reach 100% of their rolling 24h cap — but provide **no warning**. Users are suddenly locked out with no advance notice.

The `notifications.sql` worksheet deploys a custom notification system that fills this gap:

1. **Stored procedure** checks each user's rolling 24h usage against their effective limit
2. **Serverless task** runs the check every 15 minutes
3. **Email notification** is sent when any user reaches a configurable threshold (default 80%)
4. **Audit table** logs every alert and suppresses duplicates within a cooldown window

### Quick setup

```sql
-- Set daily limits first (see set-a-limit.md — Option C)
...
Then run `worksheets/notifications.sql` (Tier 2 section) to deploy the alert task, or use the notebook's Section C for an interactive walkthrough.

### What the alert looks like

Subject: `Cortex Code limit alert: alice at 85% (cli)`

> User alice has used 17.0 of 20.0 credits (85.0%) on the cli surface in the last 24 hours.
>
> Threshold: 80%
> Action: The user will be blocked when they reach 100%. Consider increasing their limit or reaching out.
...
| Want to cap individual users (required for per-user alerts) | [set-a-limit.md](set-a-limit.md) — Option C |
