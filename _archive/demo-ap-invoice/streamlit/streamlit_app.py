    ...
    if queue.empty:
        ...
        for _, row in queue.iterrows():
            with st.expander(
                ...
                # AI_EXTRACT per-field confidence scores (available for real PDF extractions)
                scores_data = session.sql(f"""
                    SELECT AI_EXTRACT_RAW:scoring:scores AS field_scores
                    FROM {SCHEMA}.INVOICE_HEADER
                    WHERE INVOICE_ID = {row['INVOICE_ID']}
                      AND AI_EXTRACT_RAW:scoring IS NOT NULL
                """).collect()
                if scores_data and scores_data[0]["FIELD_SCORES"]:
                    st.markdown("**AI Extraction Confidence** _(per-field scores from AI_EXTRACT)_")
                    try:
                        scores = json.loads(scores_data[0]["FIELD_SCORES"])
                        score_cols = st.columns(len(scores))
                        for i, (field, val) in enumerate(scores.items()):
                            s = val.get("score", 0) if isinstance(val, dict) else 0
                            label = field.replace("_", " ").title()
                            score_cols[i].metric(label, f"{s:.0%}")
                    except Exception:
                        pass
