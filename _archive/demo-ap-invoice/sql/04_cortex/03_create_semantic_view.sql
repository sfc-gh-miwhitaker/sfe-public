...
CREATE OR REPLACE SEMANTIC VIEW SNOWFLAKE_EXAMPLE.SEMANTIC_MODELS.SV_AP_INVOICE
    ...
    FACTS (
        ...
        inv.VALIDATION_SCORE  AS VALIDATION_SCORE
            ...
            COMMENT = 'Composite validation score (0.0-1.0) from AI_EXTRACT per-field confidence plus vendor matching. Threshold for auto-approval is 0.75',
        ...
        lines.GL_CODE_CONFIDENCE  AS GL_CODE_CONFIDENCE
    ...
    DIMENSIONS (
        ...
        inv.VENDOR_NAME_RESOLVED  AS VENDOR_NAME_RESOLVED
        ...
        lines.GL_CODE_SUGGESTED  AS GL_CODE_SUGGESTED
        ...
        spend.GL_CATEGORY  AS GL_CATEGORY
    ...
    METRICS (
        ...
        lines.TOTAL_LINE_SPEND AS SUM(lines.LINE_TOTAL)
            WITH SYNONYMS = ('total line item spend', 'aggregate line spend')
            COMMENT = 'Sum of all line item amounts across selected scope'
    )

    COMMENT = 'DEMO: AP Invoice Pipeline analytics for Cortex Analyst NL queries (Expires: 2026-08-12)'
    ...
    AI_SQL_GENERATION '
        ...
        For vendor-level analysis, use VENDOR_NAME_RESOLVED dimension.
        For GL-level analysis, use GL_CODE_SUGGESTED and GL_SUGGESTED_DESC dimensions.
        Default time range is current month unless specified otherwise.
    '

    AI_VERIFIED_QUERIES (
        total_spend_by_property AS (
            QUESTION 'What is the total spend by property?'
            VERIFIED_AT 1747094400
            ONBOARDING_QUESTION TRUE
            VERIFIED_BY '(STEWARD = se_community)'
            SQL 'SELECT inv.PROPERTY, SUM(inv.TOTAL_AMOUNT) AS TOTAL_SPEND FROM SNOWFLAKE_EXAMPLE.AP_INVOICE.PROCESSED_INVOICES AS inv WHERE inv.STATUS = ''PROCESSED'' GROUP BY inv.PROPERTY ORDER BY TOTAL_SPEND DESC'
        ),
        invoices_pending_review AS (
            QUESTION 'How many invoices are pending review?'
            VERIFIED_AT 1747094400
            ONBOARDING_QUESTION TRUE
            VERIFIED_BY '(STEWARD = se_community)'
            SQL 'SELECT COUNT(*) AS PENDING_REVIEW_COUNT FROM SNOWFLAKE_EXAMPLE.AP_INVOICE.PROCESSED_INVOICES AS inv WHERE inv.STATUS = ''REVIEW'''
        ),
        top_vendors_by_spend AS (
            QUESTION 'Who are the top vendors by spend?'
            VERIFIED_AT 1747094400
            ONBOARDING_QUESTION TRUE
            VERIFIED_BY '(STEWARD = se_community)'
            SQL 'SELECT inv.VENDOR_NAME_RESOLVED, SUM(inv.TOTAL_AMOUNT) AS TOTAL_SPEND, COUNT(*) AS INVOICE_COUNT FROM SNOWFLAKE_EXAMPLE.AP_INVOICE.PROCESSED_INVOICES AS inv WHERE inv.STATUS = ''PROCESSED'' GROUP BY inv.VENDOR_NAME_RESOLVED ORDER BY TOTAL_SPEND DESC LIMIT 10'
        ),
        auto_approval_rate AS (
            QUESTION 'What is the auto-approval rate?'
            VERIFIED_AT 1747094400
            ONBOARDING_QUESTION TRUE
            VERIFIED_BY '(STEWARD = se_community)'
            SQL 'SELECT ROUND(COUNT_IF(inv.STATUS = ''PROCESSED'') * 100.0 / NULLIF(COUNT(*), 0), 1) AS AUTO_APPROVAL_RATE FROM SNOWFLAKE_EXAMPLE.AP_INVOICE.PROCESSED_INVOICES AS inv'
        )
    );
